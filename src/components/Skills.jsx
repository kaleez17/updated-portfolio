...
const Skills = () => {
  ...
  // MATHEMATICAL INFINITY PATH
  const pathData = "M400,200 C550,50 750,50 750,200 C750,350 550,350 400,200 C250,50 50,50 50,200 C50,350 250,350 400,200";

  return (
    <section id="skills" className="relative z-[30] w-full min-h-screen py-32 overflow-hidden flex flex-col items-center justify-center">
      
      {/* GHOST HEADING: POWER VORTEX */}
      ...
      {/* THE VORTEX ENGINE: CSS 3D (NO CANVAS) */}
      <div className="relative w-full max-w-[800px] h-[400px] scale-[0.6] md:scale-100 transition-transform duration-700">
        
        {/* CENTER HUB: M.Sc IT CORE */}
        <div className="absolute top-[200px] left-[400px] -translate-x-1/2 -translate-y-1/2 z-30 flex flex-col items-center gap-3">
          ...
          <span className="text-[10px] font-black text-white uppercase tracking-[0.6em] bg-black/80 px-6 py-2 rounded-full border border-white/10 backdrop-blur-xl">
            M.Sc IT Specialist
          </span>
        </div>
        ...
        <svg viewBox="0 0 800 400" className="absolute inset-0 w-full h-full opacity-10 pointer-events-none">
          <path d={pathData} fill="none" stroke="white" strokeWidth="2" strokeDasharray="10,10" />
        </svg>

        {/* 3D ROUND SPHERES (CSS 3D) */}
        {techStack.map((tech, i) => (
          ...
          >
            <div className="group relative flex flex-col items-center justify-center w-full h-full p-2">
              {/* THE ROUND SPHERE EFFECT */}
              <div 
                className="w-12 h-12 md:w-16 md:h-16 rounded-full flex items-center justify-center shadow-2xl transition-all duration-500 group-hover:scale-125 border border-white/10 backdrop-blur-xl"
                style={{ 
                  background: `radial-gradient(circle at 30% 30%, ${tech.color}, #000)`,
                  boxShadow: `0 0 30px ${tech.color}40`
                }}
              >
                <tech.icon size={20} className="text-white drop-shadow-lg" />
              </div>
              
              {/* NAME LABEL */}
              <span className="mt-3 text-[7px] md:text-[9px] font-black text-white/80 uppercase tracking-tighter text-center bg-black/40 px-2 py-0.5 rounded-md backdrop-blur-sm">
                {tech.name}
              </span>
            </div>
          </motion.div>
      ...
      <div className="mt-20 text-center opacity-30">
        <p className="text-[8px] font-black uppercase tracking-[0.8em] text-white">Front End Developer</p>
      </div>
    </section>
  );
};
